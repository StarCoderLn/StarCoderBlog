...
出现这个问题的原因是，服务器上没有装 Git，执行 `yum install git` 命令，安装 Git。安装完成后，再执行 `which git` 或者 `whereis git` 命令，查看 Git 的安装路径。

3. 配置好代码仓库地址，我这里用的是 Gitee，因为 Github 实在是太慢了，代码一直拉不下来。

![jenkins](../.vuepress/public/assets/image/engineering/jenkins15.png 'jenkins')
...
![jenkins](../.vuepress/public/assets/image/engineering/jenkins16.png 'jenkins')

5. 因为我的项目是用 VuePress 搭建的，所以还需要在服务器上装好 Node 环境和安装 VuePress。

- [安装 Node](../../server/esc.md#安装-node)
...
我们在服务器上装好 Node 环境后，还需要在 Jenkins 上配置环境变量，才能正常使用。在 `系统管理 -> 系统配置 -> 全局属性 -> 环境变量` 中配置。
...
![jenkins](../.vuepress/public/assets/image/engineering/jenkins17.png 'jenkins')

此时，点击项目的立即构建按钮就可以看到构建成功啦～:rofl:

7. 每次构建前自动清空项目的工作空间。需要安装 `Workspace Cleanup Plugin` 这个插件，然后在项目配置的构建环境那里进行配置。

![jenkins](../.vuepress/public/assets/image/engineering/jenkins18.png 'jenkins')

8. 设置超时时间。需要安装 `Build Timeout` 这个插件，然后在项目配置的构建环境那里进行配置。

![jenkins](../.vuepress/public/assets/image/engineering/jenkins19.png 'jenkins')

9. 提交代码后自动进行构建部署。需要安装 `Generic Webhook Trigger Plugin` 这个插件，安装完成后配置方法如下。

- 在项目配置的构建触发器中勾选 Generic Webhook Trigger，勾选就行了，下面的东西都不用填。

![jenkins](../.vuepress/public/assets/image/engineering/jenkins20.png 'jenkins')

- 点击右上角的登录用户名，然后点左侧的设置，在 API Token 那里点击添加新 Token，再点击生成，然后把生成的 token 复制一份。

![jenkins](../.vuepress/public/assets/image/engineering/jenkins21.png 'jenkins')
