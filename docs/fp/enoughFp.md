...
![Functional Programming](../.vuepress/public/assets/image/fp/fp9.png "Functional Programming")
...
以上这两条是范畴遵循的定律，下面用一个组合函数来给出一个完整定义：
...
总之，monad 让我们深入到嵌套的运算当中，使我们能够在完全避免回调金字塔（pyramid of doom）情况下，为变量赋值，运行有序的作用，执行异步任务等等。借助 “pointed” 这个可靠的帮手，monad 能够借给我们从盒子中取出的值，而且知道我们会在结束使用后还给它。

monad 非常强大，但我们还需要一些额外的容器函数。比如，假设我们想同时运行一个列表里的 api 调用，然后再搜集返回的结果，怎么办？是可以使用 monad 实现这个任务，但必须要等每一个 api 完成后才能调用下一个。合并多个合法性验证呢？我们想要的肯定是持续验证以搜集错误列表，但是 monad 会在第一个 Left 登场的时候停掉整个演出。这就需要用到下一章讲到的 applicative functor 了。
...
**1. 应用 applicative functor**

先来看下面这个场景，假设有两个同类型的 functor，我们想把两者作为一个函数的两个参数传递过去来调用这个函数。比如让两个 Container 的值相加：
...
// 使用 map 函数试试
var container_of_add_2 = map(add, Container.of(3)) // Container(add(2))
```

此时，我们创建了一个 Container，它内部的值是一个局部调用的函数。确切来讲，就是我们想让 Container(add(2)) 中的 add(2) 应用到 Container(3) 中的3上来完成调用。也就是说，我们想把一个 functor 应用到另一个上。
...
但是，这种方式有个问题，那就是 monad 的顺序执行问题：**所有的代码都只会在前一个 monad 执行完毕之后才执行**。

因此，当遇到这种问题时，就希望能够无需借助这些不必要的函数和变量，以一种简明扼要的方式把一个 functor 的值应用到另一个上去。
...
这样的代码看起来干净整洁。有点需要强调的是，本例中的 add 是被 map 所局部调用的，所以 add 必须是一个 curry 函数。
...
记住，this._value 是一个函数，将会接收另一个 functor 作为参数，所以我们只需要 map 它。由此可以得出 applicative functor 的定义如下：

> applicative functor 是实现了 ap 方法的 pointed functor

注意 pointed 这个前提，这是非常重要的一个前提。

在深入理解这个概念之前，先来探索一个特性：

```js
F.of(x).map(f) === F.of(f).ap(F.of(x))
```

这句代码的意思就是，map 一个 f 等价于 ap 一个值为 f 的 functor。或者更好的解释是，既可以把 x 放到容器里然后调用 map(f) ，也可以同时让 f 和 x 发生 lift，然后对它们调用 ap。这让我们能够以一种从左到右的方式编写代码：
...
这个例子中，我们使用了 Task，这是 applicative functor 主要的用武之地。
...
再次强调，因为我们是使用局部调用的函数来达成上述结果的，所以必须要保证 renderPage 是 curry 函数，否则它就不会一直等到两个 Task 都完成。
...
**4. pointfree 版本的 applicative functor**
...
liftA2、liftA3 看起来是个奇怪的名字，不过要是真正理解了，它的含义就很清晰了：让那些小代码块发生 lift，成为 applicative functor 中的一员。
...
Either.of(createUser).ap(checkEmail(user)).ap(checkName(user)) // Left('invalid email')
...
```js
liftA2(add, Maybe.of(2), Maybe.of(3)) // Maybe(5)
...
本书介绍的所有这些接口都互相依赖并遵守一些定律，那么我们就可以根据一些强接口来定义一些弱接口了。
...
有一点需要指出的是，**ap 的魅力有一部分就来自于并行的能力，所以通过 chain 来定义它就失去了这种优化**。即便如此，开发者在设计出最佳实现的过程中就能有一个立即可用的接口，也是很好的。
...
applicative functor 拥有一些有用的特性。首先， applicative functor 是“组合关闭”（closed under composition）的，意味着 **ap 永远不会改变容器类型**（另一个胜过 monad 的原因）。这并不是说我们无法拥有多种不同的作用——我们还是可以把不同的类型压栈的，只不过我们知道它们将会在整个应用的过程中保持不变。
...
对一个 functor 应用 id 函数不会改变 v 里的值。比如：
...
这里有意思的一点是，就像我们之前证明了的，of/ap 等价于 map，因此这个同一律遵循的是 functor 的同一律：map(id) == id。
...
互换（interchange）表明的是选择**让函数在 ap 的左边还是右边发生 lift 是无关紧要的**。
...
组合不过是在**检查标准的函数组合是否适用于容器内部的函数调用**。
